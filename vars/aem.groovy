...
def invalidateCache(configObject) {
  configObject.dispatchers.each {dispatcher ->
    ...
    sh(script: "curl -H 'CQ-Action: Delete' -H 'CQ-Handle: /etc/designs/orion' -H 'CQ-Path: /' -H 'Content-Length: 0' -H 'Content-Type: application/octet-stream' --noproxy .com http://${dispatcher}/invalidate.cache")
  }
}
